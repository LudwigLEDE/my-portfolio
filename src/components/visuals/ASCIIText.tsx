...
class AsciiFilter {
  ...
  constructor(renderer: THREE.WebGLRenderer, { fontSize, fontFamily, charset, invert }: AsciiFilterOptions = {}) {
    ...
    this.domElement.style.overflow = 'hidden'; // Prevent scrollbars from ascii
  ...
  reset() {
    if (this.context) {
      ...
      this.pre.style.pointerEvents = 'none'; // Click through
...
class CanvAscii {
  ...
  render() {
    ...
    // --- WARP EFFECT LOGIC ---
    // We update camera Z based on window scroll
    const scrollY = window.scrollY;
    // Speed factor: 0.1 means for every 10px scrolled, camera moves 1 unit back
    const warpSpeed = 0.05; 
    this.camera.position.z = this.initialZ + (scrollY * warpSpeed);
    
    // Optional: Add some rotation or 'shake' at high speeds
    // this.mesh.rotation.z = scrollY * 0.0005;
...
export default function ASCIIText({
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver((entries) => {
      ...
      if (width > 0 && height > 0) asciiRef.current.setSize(width, height);
    });
  ...
  return (
    <div 
      ref={containerRef} 
      className="ascii-text-container w-full h-full relative"
    >
      <style>{`
